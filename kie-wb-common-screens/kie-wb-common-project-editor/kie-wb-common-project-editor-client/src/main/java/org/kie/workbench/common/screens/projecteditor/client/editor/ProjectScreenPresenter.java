...
public class ProjectScreenPresenter implements ProjectScreenView.Presenter {
	...
	public void selectedPathChanged(
			@Observes final ProjectContextChangeEvent event) {
	...
	private void init() {
		...
		projectScreenService.call(new RemoteCallback<ProjectScreenModel>() {
			@Override
	...
	private void updateEditorTitle() {
		changeTitleWidgetEvent.fire(new ChangeTitleWidgetEvent(placeRequest,
				ProjectEditorResources.CONSTANTS.ProjectScreenWithName(model
						.getPOM().getGav().getArtifactId()
						+ ":"
	...
	private void setupPathToPomXML() {
		...
		pathToPomXML = IOC.getBeanManager().lookupBean(ObservablePath.class)
				.getInstance().wrap(project.getPomXMLPath());
								...
								projectScreenService.call(
										new RemoteCallback<Void>() {
											...
											public void callback(final Void o) {
												...
												placeManager
														.forceClosePlace(placeRequest);
											}
										},
										new HasBusyIndicatorDefaultErrorCallback(
												busyIndicatorView)).delete(
										project.getPomXMLPath(), comment);
							}
						});
	...
	private Command getRenameCommand() {
		return new Command() {
			@Override
			public void execute() {
				final RenamePopup popup = new RenamePopup(
						project.getRootPath(), projectNameValidator,
						new CommandWithFileNameAndCommitMessage() {
							@Override
							public void execute(
									final FileNameAndCommitMessage details) {
								...
								projectScreenService
										.call(new RemoteCallback<ProjectScreenModel>() {
											@Override
	...
	private YesNoCancelPopup createYesNoCancelPopup() {
		return YesNoCancelPopup.newYesNoCancelPopup(
				org.kie.uberfire.client.resources.i18n.CommonConstants.INSTANCE
						.Information(), ProjectEditorResources.CONSTANTS
						.SaveBeforeBuildAndDeploy(), getYesCommand(),
				org.kie.uberfire.client.resources.i18n.CommonConstants.INSTANCE
						.YES(), ButtonType.SUCCESS, IconType.THUMBS_UP,

				getNoCommand(),
				org.kie.uberfire.client.resources.i18n.CommonConstants.INSTANCE
						.NO(), ButtonType.DANGER, IconType.THUMBS_DOWN,

				getCancelCommand(),
				org.kie.uberfire.client.resources.i18n.CommonConstants.INSTANCE
						.Cancel(), ButtonType.PRIMARY, IconType.SIGNOUT);
	}
	...
	private Command getNoCommand() {
		return new Command() {
			@Override
			public void execute() {
				view.showBusyIndicator(ProjectEditorResources.CONSTANTS
						.Building());
				build();
			}
		};
			...
			public void execute() {
				saveProject(new RemoteCallback<Void>() {
					...
					public void callback(Void v) {
						view.switchBusyIndicator(ProjectEditorResources.CONSTANTS
								.Building());
						notificationEvent.fire(new NotificationEvent(
								ProjectEditorResources.CONSTANTS
										.SaveSuccessful(pathToPomXML
												.getFileName()),
		...
		new SaveOperationService().save(pathToPomXML,
				new CommandWithCommitMessage() {
					...
					public void execute(final String comment) {
						...
						view.showBusyIndicator(CommonConstants.INSTANCE
								.Saving());
		...
		return new RemoteCallback<BuildResults>() {
			...
			public void callback(final BuildResults result) {
				if (result.getErrorMessages().isEmpty()) {
					notificationEvent.fire(new NotificationEvent(
							ProjectEditorResources.CONSTANTS.BuildSuccessful(),
							NotificationEvent.NotificationType.SUCCESS));
				} else {
					notificationEvent.fire(new NotificationEvent(
							ProjectEditorResources.CONSTANTS.BuildFailed(),
	...
	private class BuildFailureErrorCallback extends
